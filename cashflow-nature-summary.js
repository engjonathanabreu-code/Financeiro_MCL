...
async function build(){
  ...
  ensureStyle();q('#mclNatureSummary',content)?.remove();
  ...
  const box=document.createElement('section');box.id='mclNatureSummary';box.style.marginTop='28px';box.innerHTML=`<div class="card" style="padding:18px"><div class="mcl-nature-summary-head"><div><h3 style="margin:0 0 4px">Resumo por Natureza</h3><div class="muted">Somatório do mês selecionado, separado entre receitas e despesas</div></div><div class="mcl-nature-balance"><div class="muted">Saldo do mês</div><b class="${balance>=0?'kpi-positive':'kpi-negative'}">${brl(balance)}</b></div></div><div class="mcl-nature-grid">${section('Entradas por Natureza',ins,'Entrada')}${section('Saídas por Natureza',outs,'Saída')}</div></div>`;content.appendChild(box);
}
